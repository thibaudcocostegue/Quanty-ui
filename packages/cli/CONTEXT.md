...
2. Prompts for default theme (`midnight`, `fund-color`, or `custom`)
3. Installs `@quanty-ui/tokens` package
4. Creates token import in project
5. Enforces the selected theme in `main.ts`
6. For `custom`, scaffolds `src/styles/quant-theme.custom.css` and auto-imports it
...
export default defineCommand({
  ...
  async run() {
    ...
    // Inject theme enforcement into main.ts...
  }
})
```
...
Client apps generated through `quanty-ui init` are configured with a theme **imposed by the developer choice at init time**.

- No runtime theme selector is generated by the CLI
- Theme switcher remains a playground-only development feature
- Theme is applied in app entry (`main.ts`) using `data-quant-theme`
